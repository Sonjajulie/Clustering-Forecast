...
class Clusters:
    ...
    def plot_years(self):
        ...
        for i in range(self.k):
            directories_plots[i] = (f"/glade/scratch/totz/output-{self.output_label}/{self.var}/Cluster/{self.method_name}_Cluster_{self.k}/years/plots/Cluster_{i}/")
            Path(directories_plots[i]).mkdir(parents=True, exist_ok=True)
            directories_files[i] = (f"/glade/scratch/totz/output-{self.output_label}/{self.var}/Cluster/{self.method_name}_Cluster_{self.k}/years/files/Cluster_{i}/")
        ...
        for year in range(len(self.dict_standardized_pred_1D[self.var])):
            ...
            # ~ plt.savefig(f"{directories_plots[self.f[year]]}/{year:05d}_{self.var}_"
